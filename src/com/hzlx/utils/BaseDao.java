...
public abstract class BaseDao<T> {
    ...
    public T selectOne(String sql,Class clazz,Object... objs){
        ...
        try {
            ...
            if (resultSet.next()){
                ...
                //通过属性的名字找到对应的set方法，执行set方法 去完成赋值操作
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                for (int i = 0; i < fields.length; i++) {
                    ...
                    //跟据字段名--->字段名首字母大写，在前边拼接上set
                    String firesultSettChar = name.substring(0, 1).toUpperCase();
                    //方法名
                    String methodName = "set" + firesultSettChar + name.substring(1);
                    //根据拼接好的方法名获取方法对象
                    Method method = clazz.getMethod(methodName, fields[i].getType());
                    //指定对应的set方法，完成赋值
                    method.invoke(obj,resultSet.getObject(i+1));
     ...
     * @return 结果集
     */
    public Map<String,Object> selectOneForMay(String sql, Object...objects){
